...
// ─── Update profile repo via GitHub API ──────────────────────────────────────

async function pushProfileReadme(content) {
  const profileRepo = GITHUB_USERNAME; // GitHub profile README lives at <username>/<username>
  ...
  const sha = getStatus === 200 ? fileData.sha : undefined;

  const body = JSON.stringify({
    message: `chore: update profile dashboard — ${new Date().toUTCString()}`,
...
function renderProfileReadme(entries, username) {
  ...
  return `<!-- AUTO-GENERATED by repo-tracker — do not edit manually -->
<!-- Last updated: ${now} -->
